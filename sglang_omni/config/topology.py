...
def _validate_process_name_uniqueness(plan: ProcessTopologyPlan) -> None:
    ...
    # TP and SP stages expand into dedicated per-rank OS processes. Only
    # non-parallel stages participate in shared process groups; for a parallel
    # stage, `process` is the base name for its `*_tpN` or `*_spN` processes.
    # GPU colocation across those processes is validated separately below.
    sp_processes = [
        process_name
