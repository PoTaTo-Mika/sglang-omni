...
class StageGroup:
    ...
    def leader_endpoint(self) -> str:
        """Control-plane recv endpoint for the leader rank (used by Coordinator)."""
    ...
    async def wait_ready(self, timeout: float) -> None:
        """Block until every process in the group is ready or *timeout* expires."""
...
def _get_single_gpu_process_env(
    ...
    *,
    ...
    """Return the one-visible-GPU environment shared by TP and SP ranks."""
...
def get_stage_process_env(
    ...
    updates = _get_single_gpu_process_env(spec, env, mode="TP")
    ...
    updates.update(
        {
            ...
            # Each process sees one physical GPU remapped to local cuda:0.
            "LOCAL_RANK": "0",
        }
    )
    return updates
...
def _prepare_cuda_environment(
    ...
    env_updates = (
        ...
        if spec.sp_size > 1
        else get_stage_process_env(spec)
    )
    if not env_updates:
        return
