...
from sglang_omni.models.dots_tts.components.backbone.llm_inference import (
    LLMInference,
    LLMInferenceState,
)
...
# note (chenyang): training imports removed for serving-only port
# from dots_tts.training.losses import LossMasks, LossTerm, LossTerms
from sglang_omni.models.dots_tts.components.utils.logging import categorized_log as logc
from sglang_omni.models.dots_tts.components.utils.profiling import measure_inference
...
class DotsTtsModel(nn.Module):
    ...
    # note (chenyang): training forward/loss path omitted in the serving port.
    def forward(self, *args, **kwargs):
        raise RuntimeError(
            "DotsTtsModel.forward is training-only and is not available in the "
            "SGLang-Omni serving port."
        )
    ...
    def generate_latents(
        ...
        *,
        ...
        """Denormalized acoustic latents for one request, shape [1, frames, dim]."""
        latent_patches = list(
            self._generate_latents_stream(
                data,
                precision=precision,
        ...
        return torch.cat(latent_patches, dim=1)
