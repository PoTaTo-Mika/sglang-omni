...
async def test_voice_mutations_commit_in_dispatch_order() -> None:
    ...
    async with httpx.AsyncClient(
        ...
        try:
            for _ in range(100):
                if not proxy.voice_routing.requires_owner({"preset"}):
                    break
                await asyncio.sleep(0.01)
            assert not proxy.voice_routing.requires_owner({"clone"})
        finally:
        ...
        try:
            for _ in range(100):
                if not proxy.voice_routing.requires_owner({"preset"}):
                    break
                await asyncio.sleep(0.01)
            assert not proxy.voice_routing.requires_owner({"preset"})

            response = await proxy.forward_model_request(request, "/v1/audio/voices")
            assert response.status_code == 200
            assert proxy.voice_routing.requires_owner({"Clone"})
