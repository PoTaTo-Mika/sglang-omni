...
is tunable at launch with `--audio_chunking.max_concurrent_chunks`; the rest are
fixed model defaults (`--audio_chunking.max_audio_clip_s` exists too, but for
Whisper it can only lower the
