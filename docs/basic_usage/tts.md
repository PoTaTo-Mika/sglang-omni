...
| dots.tts | `examples/configs/dots_tts.yaml` | Continuation cloning only: exactly one local reference clip plus its transcript. Solver knobs `num_steps`, `guidance_scale`, `speaker_scale`, `max_audio_patches` go through `extra_body`. 48 kHz output, non-streaming |
